...
void Board::init()
{
	...
	RawBoard[1][1] = WALL;
	RawBoard[1][BOARD_SIZE] = WALL;
	RawBoard[BOARD_SIZE][1] = WALL;
	RawBoard[BOARD_SIZE][BOARD_SIZE] = WALL;
	


	// 初期配置
	// watanabe change 2017/3/30
	RawBoard[6][6] = WHITE;
	RawBoard[7][7] = WHITE;
	RawBoard[6][7] = BLACK;
	RawBoard[7][6] = BLACK;
	...
	Discs[BLACK] = 2;
	Discs[WHITE] = 2;
	Discs[EMPTY] = BOARD_SIZE*BOARD_SIZE - 8;
...
//watanabe wrote 2017/3/30
void Board::Reverse_disk(const Point& p, Color currentcolor){
  
  /*
  if(currentcolor == BLACK){
    cout<<"Black ATTACK CHANCE"<<endl; 
  }
  else{
    cout<<"White ATTACK CHANCE"<<endl;
  }
  cout<<"x="<<p.x<<endl;
  cout<<"y="<<p.y<<endl;
  */
